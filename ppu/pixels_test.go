...
func Test_fifo_mustPush(t *testing.T) {
	...
	t.Run("can push 8 times to an empty fifo", func(t *testing.T) {
		var fifo fifo[int]
		for i := range 8 {
			fifo.mustPush(0)
			assert.EqualValues(t, i+1, fifo.size)
		}
	})
	t.Run("panics on full fifo", func(t *testing.T) {
		var fifo fifo[struct{}]
		for range 8 {
			fifo.mustPush(struct{}{})
		}
...
func Test_fifo_tryPop(t *testing.T) {
	t.Run("returns false on empty fifo", func(t *testing.T) {
		var fifo fifo[struct{}]
		_, ok := fifo.tryPop()
		assert.False(t, ok)
	})
	t.Run("popping values", func(t *testing.T) {
		var fifo fifo[int]
		...
		for i := range 8 {
			...
			assert.EqualValues(t, 7-i, fifo.size)
			assert.True(t, ok)
		}

		_, ok := fifo.tryPop()
		assert.False(t, ok)
		assert.EqualValues(t, 0, fifo.size)
	})
}

func Test_fifo_clear(t *testing.T) {
	var fifo fifo[struct{}]
